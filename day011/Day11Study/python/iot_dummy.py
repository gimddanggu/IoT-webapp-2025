...
for day in range(total_days): # 10000일 반복
    ...
    for i in range(records_per_day): # 100번 반복
        ...
        temp = round(random.uniform(19.0, 28.0), 1) # 19~28도 사이의 온도를 랜덤으로 하고, 소수점 1번째 자리에서 반올림
        humid = round(random.uniform(30.0, 60.0), 1) # 30% ~ 60% 사이 습도 랜덤, 소수점 1번째 반올림
...
if batch:
    ...
    conn.commit()
    batch.clear() # 다 끝나서 굳이 클리어 안 해도 됨
